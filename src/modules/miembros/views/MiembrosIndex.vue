<template>
  <main class="h-full overflow-y-auto">
    <div class="container px-6 mx-auto grid">
      <!--  -->
      <!-- titulo -->
      <h1 class="mb-1 text-4xl font-bold dark:text-gray">Gestion Miembros</h1>
      <BreadCrumb home="Miembros" modulo="Gestion" />
      <div class="grid grid-cols-2 my-6 font-bold dark:text-gray">
        <div class="flex items-center">
          <h2 class="mb-1 text-4xl font-bold dark:text-gray">Listado de Miembros</h2>
        </div>
        <div>
        ...
        <div class="w-full overflow-x-auto">
          <table class="w-full whitespace-no-wrap border-black dark:border-slate-300">
            <thead>
              ...
              >
                <th class="px-6 py-3">ID Miembros</th>
            ...
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
              ...
              >
                  ...
                  <div class="flex items-center text-sm">
                    ...
                    <div class="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                      <div
                        ...
                        aria-hidden="true"
                      >
                        <p class="font-semibold">{{ miembro.IDMIEMBRO }}</p>
                      </div>
                    </div>
                  </div>
                </td>
                ...
                <td class="px-4 py-3 text-sm">
                  <div class="flex">
                    <BotonEditar :ruta="`/miembros/actualizar/${miembro.IDMIEMBRO}`"></BotonEditar>
                  </div>
                </td>
              </tr>
            </tbody>
...
import SevicesMiembros from '@/modules/miembros/services'
interface Miembros {
  IDMIEMBRO: number
  NOMBRE: string
  CARGO: string
...
const miembros = ref<Miembros[]>([])

const cargarDatos = async () => {
  const respuesta = await SevicesMiembros.MostrarMiembros()
  if (respuesta.status == 200) {
    miembros.value = Array.isArray(respuesta.data) ? respuesta.data : [respuesta.data]
