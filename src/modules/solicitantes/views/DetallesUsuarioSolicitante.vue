  ...
  <div class="min-h-screen bg-gray-100 dark:bg-gray-900 py-10 px-6">
    ...
    <div class="max-w-3xl mx-auto bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6">
      ...
      <div class="text-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Perfil del Solicitante</h1>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
        ...
        <!-- Cargo -->
        <div>
          <label class="block text-gray-700 dark:text-gray-300 font-semibold mb-2">Tipo de Solicitante:</label>
          <p class="bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-100 rounded-md p-2">
            {{ formData.TIPODESOLICITANTE }}
          </p>
        </div>
        <!-- Email -->
        <div>
        ...
        <div>
          <label class="block text-gray-700 dark:text-gray-300 font-semibold mb-2">Celular:</label>
          <p class="bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-100 rounded-md p-2">
            {{ formData.CELULAR }}
          </p>
        </div>
...
import servicesSolicitantes from '../services'
...
const cargarDatos = async () => {
  const respuesta = await servicesSolicitantes.DetallesUsuarioSolicitante()

  if (respuesta.status == 200) {
    let datos = respuesta.data[0]
    ;(formData.NOMBRE = datos.NOMBRE),
      (formData.IDENTIFICACION = datos.IDENTIFICACION),
      (formData.TIPODESOLICITANTE = datos.TIPODESOLICITANTE),
      (formData.EMAIL = datos.EMAIL)
      (formData.CELULAR = datos.CELULAR)
...
const ActualizarPassword = async () => {
  ...
  try {
    ...
    if (respuesta.status == 201) {
      ...
      ;(formDataPassword.contrasenaActual = ''), (formDataPassword.nuevaContrasena = '')
    } else if (respuesta.status == 400) {
      alert(respuesta.error?.message)
    } else if (respuesta.status == 500) {
      alert(respuesta.error?.message)
