<template>
  <main class="h-full overflow-y-auto">
    <div class="container px-6 mx-auto grid">
      ...
      />
      <AgregarLink titulo="Sesion" ruta="" :visible="false" accion=""></AgregarLink>
...
watch(
  ...
  (newID) => {
    if (newID) {
      CargarSesiones(newID) // Llamar a la función con el nuevo ID
    }
  },
  { immediate: true } // Llamar al watcher al inicializar
)
...
const CargarSesiones = async (Id: number) => {
  if (Id !== null) {
    const respuesta = await SesionService.mostrarSesionesSolicitante(Id)
