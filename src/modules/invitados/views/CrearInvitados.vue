<template>
  <div class="max-w-md mx-auto p-6 mt-20 bg-slate-50 shadow-md rounded-md">
    <h2 class="text-2xl font-bold mb-4 text-center text-gray-800">Registro de Invitado</h2>
    <form @submit.prevent="registrarInvitado" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
      <div class="mb-4">
      ...
      <div class="mb-4">
        <label class="block text-gray-700 text-sm font-bold mb-2" for="DEPENDENCIA">Dependencia</label>
        <input
...
import  Registrarinvitados  from '@/modules/invitados/services'; // Asegúrate de importar correctamente el servicio
import { ref } from 'vue'
...
const registrarInvitado = async () => {
  try{
    ...
    const respuesta = await Registrarinvitados.Registrarinvitados(
      formData.NOMBRE,
